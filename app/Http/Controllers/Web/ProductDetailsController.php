...
class ProductDetailsController extends Controller
{
    ...
    public function getDefaultTheme(string $slug): View | RedirectResponse
    {
        ...
        if ($product) {
            ...
            $moreProductFromSeller->getCollection()->transform(function ($product) {
    ...
    public function getVariantProductPrice(Request $request)
    {
        ...
        if ($product && $product->variation) {
            $product_variation = json_decode($product->variation, true);
            $product_price     = null;
            $product_sku       = null;
            $product_qty       = null;

            foreach ($product_variation as $variation) {
                $typeParts = explode('-', $variation['type']); // split "WhiteSmoke-1-2-22"

                // Extra Safe: Check if size part exists
                if (in_array($selected_size, $typeParts)) { 
                    $product_price = $variation['price'];
                    $product_sku = $variation['sku'];
                    $product_qty = $variation['qty'];
                    break;
                }
            }
            ...
            if ($product_price !== null) {
                return response()->json([
                    'price' => webCurrencyConverter($product_price),
                    'sku'   => $product_sku,
                    'qty'   => $product_qty,
                ]);
            } else {
                return response()->json(['error' => 'Selected size not found in variations.'], 404);
            }
        }

        return response()->json(['error' => 'Product not found or has no variations.'], 404);
    }
