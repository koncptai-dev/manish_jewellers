...
class ProductService
{
    ...
    public function getSkuCombinationView(object $request, object $product = null): string
    {
        ...
        //     $first = $result[0];
        //     if(isset($first[0]) && is_numeric($first[0]) && $first[0] > 1) {
        //         $request['unit_price']  = $request['unit_price'] / $first[0];
        //     }elseif(isset($first[1]) && is_numeric($first[1]) && $first[1] > 1){
        //         $request['unit_price']  = $request['unit_price'] / $first[1];
        //     }
        // }

        $choice_option = $request->has('choice_options_1') ? $request->choice_options_1 : [];
        if (isset($choice_option[0])) {
            $weights = explode(',', $choice_option[0]);
        }
        if (isset($weights) && count($weights) > 0) {
            if($weights[0] != 1){
                $request['unit_price'] = $unitPrice / $weights[0];
            }
        }
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        return [
            ...
            'purchase_price' => 0,
            'hallmark_charges'=> $request->has('hallmark_charges') ? $request['hallmark_charges'] : 0,
            'making_charges' => isset($request['making_charges']) ? $request['making_charges'] : 0,
            'product_metal' => isset($request['product_metal']) ? $request['product_metal'] : "Gold",
    ...
    public function getUpdateProductData(object $request, object $product, string $updateBy): array
    {
        ...
        $dataArray = [
            ...
            'details' => $request['description'][array_search('en', $request['lang'])],
            'colors' => $this->getColorsObject(request: $request),
            'choice_options' => json_encode($this->getChoiceOptions(request: $request)),
        ...
        if ($updateBy == 'seller' && getWebConfig(name: 'product_wise_shipping_cost_approval') == 1 && $product->shipping_cost != currencyConverter($request->shipping_cost)) {
            ...
            $dataArray += [
                'shipping_cost' => currencyConverter(amount: $request['shipping_cost']),
            ];
        }
