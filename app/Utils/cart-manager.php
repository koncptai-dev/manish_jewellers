...
$orderWiseShippingCost = 0;
    ...
    public static function addToCartPhysicalProduct($request, $product, $shippingType, $sellerShippingList): array
    {
        ...
        $price = Helpers::calculatePrice(json_decode($product->choice_options), $product->unit_price, 0, $product->product_metal);
        if ($string != null) {
        ...
        $tax = 0; //Helpers::tax_calculation(product: $product, price: $price, tax: $product['tax'], tax_type: 'percent');
        ...
        if ($request['buy_now'] == 1) {
            ...
            } elseif (($shippingType == 'category_wise' || $shippingType == 'product_wise')) {
                $cart->update([
                    'is_checked' => 1,
