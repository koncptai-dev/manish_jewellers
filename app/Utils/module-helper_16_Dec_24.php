...
if (!function_exists('digital_payment_success')) {
    function digital_payment_success($paymentData)
    {
        if (isset($paymentData) && $paymentData['is_paid'] == 1) {
            ...
            $paymentOf = $additionalData['payment_of'] ?? null;

            if ($paymentOf && $paymentOf == "installment_payment") {
                $data = [
                    'request' => [
                        'customer_id' => $additionalData['customer_id'],
                        'payment_request_from' => $additionalData['payment_request_from'],
                        'installment_payment_id' => $additionalData['installment_payment_id'],
                        'plan_code' => $additionalData['plan_code'],
                        'plan_amount' => $additionalData['plan_amount'],
                        'plan_category' => $additionalData['plan_category'],
                        'total_yearly_payment' => $additionalData['total_yearly_payment'],
                        'total_gold_purchase' => $additionalData['total_gold_purchase'],
