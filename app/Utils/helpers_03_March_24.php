...
class Helpers
{
    ...
    public static function set_data_format($data)
    {
        ...
        $data['unit_price'] = Helpers::calculatePrice($data);
    ...
    public static function calculatePrice($data)
    {
        ...
        // Extract choices for weight and carat from the product data
        ...
        if ($weight <= 0 || $carat <= 0) {
            ...
            // throw new Exception("Invalid weight or carat specified.");
        }

        // Calculate the price based on carat
        switch ($carat) {
            case 24:
                $unitPrice = (new GoldRate())->calculatePriceWithMarkup($price_gram_24k, $weight);
                break;

            case 22:
                $price_gram_22k = (new GoldRate())->calculate22CaratPrice($price_gram_24k);
                $priceWithGST = ((new GoldRate())->calculatePriceWithMarkup($price_gram_22k, $weight) + 2000);
                $unitPrice = $priceWithGST + 2000;
                break;

            case 18:
                $price_gram_18k = (new GoldRate())->calculate18CaratPrice($price_gram_24k);
                $priceWithGST = (new GoldRate())->calculatePriceWithMarkup($price_gram_18k, $weight);
                $unitPrice = $priceWithGST + 2000;
                break;
