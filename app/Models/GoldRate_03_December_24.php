...
class GoldRate extends Model
{
    ...
    public function getGoldRateDataUsingApi()
    {
        ...
        $curr = "INR";
        // $date = "/20241126";
        $date = '/' . date('Ymd', strtotime('-3 day')); // Yesterday date
    ...
    public function calculatePriceWithMarkup($pricePerGram, $grams)
    {
        $totalPrice = $pricePerGram * $grams; // Base price
        $markup = $totalPrice * 0.03;        // Calculate 3% markup
    ...
    {
        $price18Carat = $price24Carat * 0.72; // Calculate 92% of 24-carat price
