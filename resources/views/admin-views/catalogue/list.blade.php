...
@section('title', translate('catalogue_List'))
...
@section('content')
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="h1 mb-0 d-flex gap-2">
                ...
                {{ translate('catalogue_List') }}
                <span class="badge badge-soft-dark radius-50 fz-14">{{ $catalogues->total() }}</span>
            </h2>
        </div>
        <div class="row mt-20">
            <div class="col-md-12">
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="datatableSearch_" type="search" name="searchValue" class="form-control"
                                            placeholder="{{ translate('search_by_catalogue_name') }}" aria-label="{{ translate('search_by_catalogue_name') }}" value="{{ request('searchValue') }}" required>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 text-start">
                                ...
                                @foreach($catalogues as $key => $catalogue)
                                    <tr>
                                        <td>{{ $catalogues->firstItem() + $key  }}</td>
                                        <td>{{ $catalogue->name }}</td>
                                        <td class="overflow-hidden max-width-100px">
                                            {{-- <span data-toggle="tooltip" data-placement="right" title="{{$catalogue['defaultname']}}">
                                                 {{ Str::limit($catalogue['defaultname'],20) }}
                                            </span> --}}
                                        </td>
                                        <td></td>
                                        ...
                                        <td>
                                            <form action="{{route('admin.catalogue.status-update') }}" method="post" id="catalogue-status{{$catalogue['id']}}-form">
                                                @csrf
                                                <input type="hidden" name="id" value="{{$catalogue['id']}}">
                                                <label class="switcher mx-auto">
                                                    <input type="checkbox" class="switcher_input toggle-switch-message" name="status"
                                                           id="catalogue-status{{ $catalogue['id'] }}" value="1" {{ $catalogue['status'] == 1 ? 'checked' : '' }}
                                                           data-modal-id = "toggle-status-modal"
                                                           data-toggle-id = "catalogue-status{{ $catalogue['id'] }}"
                                                           data-on-image = "brand-status-on.png"
                                                           data-off-image = "brand-status-off.png"
                                                           data-on-title = "{{ translate('Want_to_Turn_ON').' '.$catalogue['defaultname'].' '. translate('status') }}"
                                                           data-off-title = "{{ translate('Want_to_Turn_OFF').' '.$catalogue['defaultname'].' '.translate('status') }}"
                                                           data-on-message = "<p>{{ translate('if_enabled_this_catalogue_will_be_available_on_the_website_and_customer_app') }}</p>"
                                                           data-off-message = "<p>{{ translate('if_disabled_this_catalogue_will_be_hidden_from_the_website_and_customer_app') }}</p>">
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                <a class="btn btn-outline-info btn-sm square-btn" title="{{ translate('edit') }}"
                                                    href="{{ route('admin.catalogue.edit', [$catalogue['id']]) }}">
                                                    <i class="tio-edit"></i>
                                                </a>
    ...
    <span id="route-admin-brand-status-update" data-url="{{ route('admin.catalogue.status-update') }}"></span>
    {{-- <span id="get-brands" data-brands="{{ json_encode($brands) }}"></span> --}}
    <div class="modal fade" id="select-catalogue-modal" tabindex="-1" aria-labelledby="toggle-modal" aria-hidden="true">
